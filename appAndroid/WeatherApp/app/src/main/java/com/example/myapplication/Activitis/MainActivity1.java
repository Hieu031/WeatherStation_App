...
public class MainActivity1 extends AppCompatActivity {
    ...
    ImageView imageView;
    private static final String CH_MONITOR = "weather_monitor";
    private static final int REQ_NOTI = 1001;
    private DatabaseReference databaseReference;
    private Double coValueCache = null, buiValueCache = null; // để gọi checkDangerousLevel an toàn
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ensureChannel();
        askNotiPermissionAndStart();

        initRecyclerview();
        setVariable();
        setVariable1();

        FirebaseDatabase database = FirebaseDatabase.getInstance("https://autosar01-default-rtdb.asia-southeast1.firebasedatabase.app");
        databaseReference = database.getReference();

        DatabaseReference myRefHumid   = databaseReference.child("Humidity");
        DatabaseReference myRefNhietDo = databaseReference.child("Temperature");
        DatabaseReference myRefBui     = databaseReference.child("Dust");
        DatabaseReference myRefCo      = databaseReference.child("CO");
        DatabaseReference myRefRain    = databaseReference.child("Rain");

        // --- RAIN ---
        myRefRain.addValueEventListener(new ValueEventListener() {
...
// --- HUMIDITY ---
        myRefHumid.addValueEventListener(new ValueEventListener() {
            @Override public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                PHumid = humid;
                tvHumid.setText(humid == null ? "--" : humid + "%");
                try {
                    if (humid != null && Double.parseDouble(humid) < 70) {
...
// --- TEMPERATURE ---
        myRefNhietDo.addValueEventListener(new ValueEventListener() {
            @Override public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                PTemp = temp;
                tvTemp.setText(temp == null ? "--" : temp + "°C");
                try {
                    if (temp != null && Double.parseDouble(temp) < 15) {
...
// --- DUST (key: "Dust") ---
        myRefBui.addValueEventListener(new ValueEventListener() {
            @Override public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                PBui = Bui;
                tvBui.setText(Bui == null ? "--" : Bui + "µg/m³");

                try { buiValueCache = (Bui == null) ? null : Double.parseDouble(Bui); }
                catch (Exception e) { buiValueCache = null; }

                if (coValueCache != null && buiValueCache != null) {
                    checkDangerousLevel(coValueCache, buiValueCache);
                }

                try {
                    if (Bui != null && Double.parseDouble(Bui) > 65) {
...
// --- CO ---
        myRefCo.addValueEventListener(new ValueEventListener() {
            @Override public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                PCo = Co;
                tvCo.setText(Co == null ? "--" : Co + "ppm");

                try { coValueCache = (Co == null) ? null : Double.parseDouble(Co); }
                catch (Exception e) { coValueCache = null; }
                ...
                try {
                    if (Co != null && Double.parseDouble(Co) > 650) {
                        tvtb.setText("Lời khuyên :Không nên ra ngoài");
                        return;
                    }
                } catch (Exception ignored) {}
    ...
    // Tạo channel (gọi 1 lần)
    private void ensureChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel ch = new NotificationChannel(
                    CH_MONITOR, "Giám sát thời tiết",
    ...
    // Cập nhật/ghi đè thông báo nhiệt độ (dùng cùng ID)
    private void updateTempNotification(double temp) {
        NotificationCompat.Builder b = new NotificationCompat.Builder(this, CH_MONITOR)
                .setSmallIcon(R.drawable.icon) // đổi thành icon sẵn có của bạn
                .setContentTitle("Giám sát nhiệt độ")
                .setContentText(String.format(java.util.Locale.getDefault(), "Hiện tại: %.1f°C", temp))
                .setOnlyAlertOnce(true)   // cập nhật im lặng, không rung/ting mỗi lần
                .setOngoing(true)         // muốn ghim thông báo (tuỳ chọn)
        ...
        NotificationManagerCompat.from(this).notify(REQ_NOTI, b.build());
    }

    // Parse an toàn (tránh NumberFormatException khi giá trị null / "null")
    private Double tryParseDouble(Object v) {
        if (v == null) return null;
        try { return Double.parseDouble(String.valueOf(v)); }
        catch (Exception e) { return null; }
    }
    private void askNotiPermissionAndStart() {
    ...
    private void startTempMonitorService() {
        Intent svc = new Intent(this, FirebaseMonitoringService.class);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
