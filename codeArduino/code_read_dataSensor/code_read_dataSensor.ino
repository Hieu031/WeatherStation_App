...
//   Serial.printf("Sending data to Firebase successfully!\n"");
//   bool ok1 = Firebase.RTDB.setJSON(&fbdo, path.c_str(), &json);
//   bool ok2 = Firebase.RTDB.setJSON(&fbdo, "/WeatherCurrent", &json); 
//   if (ok1 && ok2) {
//     Serial.printf("OK: %s\n", timestamp.c_str());
...
void sendDataToFirebase(SensorData data){
  ...
  /* Send data currently */
  if(!Firebase.RTDB.setString(&fbdo, "/WeatherCurrent/Temperature", String(data.temperature))){
    Serial.printf("Temperature err: %s\n", fbdo.errorReason().c_str());
  }
  if(!Firebase.RTDB.setString(&fbdo, "/WeatherCurrent/Humidity", String(data.humidity))){
    Serial.printf("Humidity err: %s\n", fbdo.errorReason().c_str());
  }
  if(!Firebase.RTDB.setString(&fbdo, "/WeatherCurrent/Rain", String(data.rain))){
    Serial.printf("Rain err: %s\n", fbdo.errorReason().c_str());
  }
  if(!Firebase.RTDB.setString(&fbdo, "/WeatherCurrent/CO", String(data.co))){
    Serial.printf("CO err: %s\n", fbdo.errorReason().c_str());
  }
  if(!Firebase.RTDB.setString(&fbdo, "/WeatherCurrent/Dust", String(data.dust))){
    Serial.printf("Dust err: %s\n", fbdo.errorReason().c_str());
  }
